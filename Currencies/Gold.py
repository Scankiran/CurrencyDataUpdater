...
def returnBank():
    rs = requests.get("https://altin.doviz.com/gram-altin")
    ...
    for i in range(1,4):
        ob = CurrencyBase(title[i], cells[i].get_text(),cells[i].get_attribute_list('class')[1].split('-')[1])
...
def run():
    changeList, bankList, currencyInfo = returnBank()
    changeRates = list()
    bankValues = list()
    ...
    return [changeRates,bankValues,currencyInfo]